...
We calculated total cases/deaths, and confirmed cases/deaths, then estimate CFR with a simple binomial calculation.

```{r}
total_cases <- MVD_linelist %>% summarise(total_cases = length(Onset_week)) 
total_confirmed_cases <- MVD_linelist %>% filter(Type == "confirmed") %>% summarise(total_cases = length(Onset_week)) 
total_deaths <- MVD_linelist %>% filter(Status == "dead") %>% summarise(total_deaths = length(Onset_week))
total_confirmed_deaths <- MVD_linelist %>% filter(Status == "dead" & Type == "confirmed") %>% summarise(total_deaths = length(Onset_week)) 
...
ci_text(total_deaths,total_cases)

# CFR among confirmed cases
ci_text(total_confirmed_deaths,total_confirmed_cases)
```
...
# Calculate CFR
total_deaths_ebola <- sum(!is.na(data_outcome$Date_Death))
total_cases_ebola <- nrow(data_outcome)
ci_text(total_deaths_ebola,total_cases_ebola)
```

### Estimating underascertainment of cases
...
#### Case study: Marburg in Equatorial Guinea, 2023
For Marburg, the overall CFR calculated above was 88%. Based on a systematic review by [Nyakarahuka et al](https://bmcinfectdis.biomedcentral.com/articles/10.1186/s12879-016-2045-6), we assume that the 'true' CFR for Marburg is 65% (95% CI: 54-75%).

We can then estimate _p_ = tCFR/oCFR as
```{r}
65/as.numeric(total_deaths/total_cases)
```

_XX Need to consider how to extract normal distribution from CFR (update epiparameter?) and propagate uncertainty XX _
...
We can then estimate _p_ = tCFR/oCFR as
```{r}
53/as.numeric(total_deaths_ebola/total_cases_ebola)
```

Because p is greater than 100%, it suggests little underascertainment relative to the 2001 outbreak.
...
First, we can use the `extract_param` function in epiparameter to estimate the Marburg onset-to-death delay and store this as an epidist object.
...
Suppose hypothetically we had a scenario where by 1st March 2023, 18 deaths had been reported in the outbreak. We can therefore use `known_outcomes` in datadelay to estimate the number of cases that have a known outcome by this point, and compare with a 'naive' estimate that doesn't account for delays.
...
# Add death column XX NOTE TO EDIT BELOW ONCE datadelay::known_outcomes updated on GitHub XX
MVD_linelist_cut$Death_week <- MVD_linelist_cut$Onset_week

# Convert to incidence
MVD_cases_deaths <- incidence2::incidence(MVD_linelist_cut,c("Onset_week","Death_week")) |> complete_dates()
...
MVD_cases_deaths <- MVD_cases_deaths[,c("date_index","cases","deaths")] # Reorder for function
MVD_cases_deaths <- as.data.frame(MVD_cases_deaths)
  
MVD_cases_deaths |> filter(date_index<cut_off)
known_outcomes_MVD <- datadelay::known_outcomes(MVD_cases_deaths, marburg_onset_death)

# Calculate cases with known outcomes to date
total_known_to_date <- sum(known_outcomes_MVD$known_outcomes)
```

Comparing CFR calculated using the raw number of cases to date, and the number with known outcomes (i.e. adjusting for delays), we obtain the following estimates of CFR:
...
_XX Need to consider how `datadelay` can use weeks as input rather than days XX _

### 3. From case and death timeseries

If we have the full incidence data for of cases and deaths available, we can also use `datadelay` to estimate the CFR, adjusting for delay from onset to outcome. An example with the 1976 Ebola dataset is reported in the [datadelay vignette](https://epiverse-trace.github.io/datadelay/articles/estimate_static_severity.html), but here we show how this method performs against the 'ground truth' of subsequent known outcomes in the line list, as well as an alternative estimation method based on `EpiNow2`.
...
First we estimate the CFR based on the line list data up to 5th October 1976. We perform two calculations: 1) Estimate CFR based only on cases with known outcomes up to this date. 2) Estimate a 'ground truth' CFR with the benefit of hindsight by following up all cases that had onsets prior to this date, but outcomes that were not necessarily known by 5th October:
```{r}
...
We then estimate the overall CFR with datadelay and EpiNow2, with and without adjusting for delays:

```{r}
# Calculate real-time overall CFR with datadelay, without adjusting for delays
dt_ncfr_static_ebola <- estimate_static(
  df_ebola_subset,
  correct_for_delays = FALSE) |> 
  format_output(estimate_type = "severity",type = "Naive")

# Calculate real-time overall CFR with datadelay, adjusting for delays
...
```{r}
# Compare line list, datadelay and EpiNow2 estimates for CFR:
dt_static_clean_ebola <- rbind(c(dt_ncfr_static_ebola$Estimate,"naive incidence calculation"),
                               c(dt_ccfr_static_ebola$Estimate,"datadelay adjusted estimate"),
                               c(dt_ccfr_static_ebola_epinow2,"EpiNow2 estimate"),
