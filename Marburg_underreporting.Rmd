...
# Define confidence interval summary:
ci_text_vec<-function(x){
  y <- signif(x,2)
  paste(y[1],"% (95% CI: ",y[2],"-",y[3],"%)",sep="")
}
...
Suppose hypothetically we had a scenario where by 1st March 2023, 18 deaths had been reported in the outbreak. We can therefore use `known_outcomes` in cfr to estimate the number of cases that have a known outcome by this point, and compare with a 'naive' estimate that doesn't account for delays.
...
# Add death column XX NOTE TO EDIT BELOW ONCE cfr::known_outcomes updated on GitHub XX
...
known_outcomes_MVD <- cfr::known_outcomes(MVD_cases_deaths, marburg_onset_death)
...
_XX Need to consider how `cfr` can use weeks as input rather than days XX _
...
```{r}
# Remove EpiNow2 example for now
...
dt_ncfr_static_ebola <- cfr::estimate_static(
  df_ebola_subset,
...
dt_ccfr_static_ebola <- cfr::estimate_static(
  df_ebola_subset,
...
param_ebola <- get_parameters(onset_to_death_ebola)
...
# Format data for fitting - remove EpiNow example for now
...
dt_static_clean_ebola <- rbind(c(dt_ncfr_static_ebola,"naive incidence calculation"),
                               c(dt_ccfr_static_ebola,"cfr adjusted estimate"),
