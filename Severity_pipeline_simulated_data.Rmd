...
```{r}
library(incidence2)
incidence_g3 <- incidence2::incidence(linelist_3,"onset_date")
deaths_g3 <- incidence2::incidence(linelist_3,"death_date")
...
```{r}
cases_deaths2 <- Linelist_group3[,c(6,8)]
names(cases_deaths2)[1:2] = c("date","deaths")
cases_deaths2 <- cases_deaths2 %>% mutate(cases = rep(1, nrow(cases_deaths2)), .after = "date")
```
